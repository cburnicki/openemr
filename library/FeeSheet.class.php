...
class FeeSheet
{
  ...
  //  pricelevel
    public function addServiceLineItem($args)
    {
        ...
        if (!$code_text) {
            ...
            if (!isset($args['fee'])) {
                ...
                $prdefault = null;
                $prrow = null;
                $prrecordset = sqlStatement($query, [$codes_id]);
    ...
    public function addProductLineItem($args, $convert_units = true)
    {
        ...
        if (!isset($args['fee'])) {
            ...
            $prdefault = null;
            $prrow = null;
            $prrecordset = sqlStatement($query, [$drug_id, $selector]);
            while ($row = sqlFetchArray($prrecordset)) {
                if (empty($prdefault)) {
                    $prdefault = $row;
                }
  ...
  //
    public function loadProductItems()
    {
        ...
        "ORDER BY ds.sale_id";
        $sres = sqlStatement($query, [$this->pid, $this->encounter]);
    ...
    public function save(
            ...
            foreach ($bill as $iter) {
                ...
                if ($code_type == 'COPAY') {
                    ...
                    if ($id) {
                        ...
                        $res_amount = sqlQuery(
                            ...
                            [$this->pid, $this->encounter, $session_id]
                        );
                ...
                if ($id) {
                    ...
                    if ($del) {
                        ...
                        if (!empty($tmp)) {
                              $tmparr = ['code' => $code, 'authorized' => $auth];
                            if (isset($iter['units'    ])) {
                                $tmparr['units'      ] = $units;
                            }
                            ...
                            foreach ($tmparr as $key => $value) {
                                if ($tmp[$key] != $value) {
                                    ...
                                    sqlStatement("UPDATE billing SET `$key` = ? WHERE id = ?", [$value, $id]);
                                }
                            }
                        }
                    }
                } elseif (!$del) { // Otherwise it's a new item...
                    $logarr = [$code_type, $code, '', $pricelevel, $fee, $units, $provid, ''];
        ...
        if ($copay_update == true && $update_session_id != '' && $mod0 != '') {
            sqlStatement(
                ...
                [$ct0, $cod0, $mod0, $this->pid, $this->encounter, $update_session_id]
            );
        }
                        ...
                        if (!empty($tmprow)) {
                            ...
                            if (!empty($tmprow['inventory_id'])) {
                                sqlStatement(
                                    ...
                                    [$tmprow['quantity'], $tmprow['inventory_id']]
                                );
                            }
                        }
                            ...
                            foreach (
                                ...
                                if ($tmprow[$key] != $value) {
                                    ...
                                    if ($key == 'quantity' && $tmprow['inventory_id']) {
                                        sqlStatement(
                                            "UPDATE drug_inventory SET on_hand = on_hand - ? WHERE inventory_id = ?",
                                            [$inv_units - $tmprow['quantity'], $tmprow['inventory_id']]
                                        );
                                    }
                                }
                            }
        ...
        if (isset($main_provid) && $main_provid != $this->provider_id) {
            ...
            sqlStatement(
                ...
                [$main_provid, $this->pid, $this->encounter]
            );
        ...
        if (isset($main_supid) && $main_supid != $this->supervisor_id) {
            sqlStatement(
                ...
                [$main_supid, $this->pid, $this->encounter]
            );
    ...
    public function genCodeSelectorValue($codes)
    {
        ...
        if ($codetype == 'PROD') {
            $crow = sqlQuery(
                ...
                "LIMIT 1",
                [$this->pid, $this->encounter, $code]
            );
            $this->code_is_in_fee_sheet = !empty($crow['sale_id']);
            $cbarray = [$codetype, $code, $selector];
        } else {
            $crow = sqlQuery(
                ...
                [$this->pid, $this->encounter, $codetype, $code_types[$codetype]['id'], $code]
            );
            $this->code_is_in_fee_sheet = !empty($crow['id']);
            $cbarray = [$codetype, $code];
        }
